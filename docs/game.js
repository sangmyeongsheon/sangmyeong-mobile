...
function updateInfoPanel() {
  ...
  if (!gameOver) {
    turnMsg =
      currentPlayer === HUMAN_PLAYER ? "당신 턴 (빨강)" : "AI 턴 (파랑)";
  }
...
function runAiTurn() {
  ...
  if (aiMove) {
    ...
    gameMessage = `AI 이동: (${aiMove.sx + 1},${aiMove.sy + 1}) → (${
      aiMove.tx + 1
    },${aiMove.ty + 1})`;
    lastPlayerPassed = false;
  } else {
...
function checkForPassOrGameOver() {
  ...
  if (!currentPlayerMoves.length) {
    if (currentPlayer === HUMAN_PLAYER)
      gameMessage = "움직일 수 있는 말이 없습니다. 턴이 자동으로 넘어갑니다.";
